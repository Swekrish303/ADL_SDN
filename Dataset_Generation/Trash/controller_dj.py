...
import Trash.topo_fstr as topo_fstr
...
class DijkstraSwitch(object):
    ...
    def __init__(self, connection, dpid, topo):
        ...
        self.dpid = dpid
        self.topo = topo
        self.name = topo_fstr.dpid_to_name(self.dpid)

        # destination_ip to output port mapping for this node
        self.table = {} 
        ...
        for host in self.topo.hosts():
            ...
            self.table[topo_fstr.host_to_ip(host)] = port
...
class install_dj(object):
    ...
    def _handle_ConnectionUp(self, event):
        ...
        name = topo_fstr.dpid_to_name(event.dpid)
        ...
        for host in self.topo.hosts():
            ...
            msg.match.nw_dst = IPAddr(topo_fstr.host_to_ip(host))
            msg.actions.append(of.ofp_action_output(port=port))
            connection.send(msg)
...
def launch(topo=None, install=False):
    ...
    k = 4
    topo = topo_fstr.topos[topo_name](int(k))
